...
public class AuthManagerImpl : IAuthManager
{
    ...
    {
        var loginUser = await _blazorIUserService.GetUserAsync(username); // Get user from database
        ...
        var principal = CreateClaimsPrincipal(loginUser); // convert user object to ClaimsPrincipal
    ...
    {
        var loginUser = await GetUserFromCacheAsync(); // retrieve cached user, if any

        var principal = CreateClaimsPrincipal(loginUser); // create ClaimsPrincipal
    ...
    private async Task<LoginUser?> GetUserFromCacheAsync()
    {
        ...
        var loginUser = JsonSerializer.Deserialize<LoginUser>(userAsJson)!;
        return loginUser;
    }

    private static void ValidateLoginCredentials(string password, LoginUser? loginUser)
    {
        if (loginUser == null) throw new Exception("Username not found");

        if (!string.Equals(password, loginUser.Password)) throw new Exception("Password incorrect");
    }
    ...
    private static ClaimsPrincipal CreateClaimsPrincipal(LoginUser? loginUser)
    {
        if (loginUser != null)
        {
            var identity = ConvertUserToClaimsIdentity(loginUser);
    ...
    {
        var serialisedData = JsonSerializer.Serialize(loginUser);
